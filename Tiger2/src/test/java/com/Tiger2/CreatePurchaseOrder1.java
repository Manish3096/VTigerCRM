...
import com.Tiger2.Baseclass;

public class CreatePurchaseOrder1 extends Baseclass {
@Test
public void toCreatePurchaseOrder1() throws EncryptedDocumentException, IOException, InterruptedException, InvalidFormatException
{
   ...
   String qty = f.getExcelData("Createdetails", 1,3);
    ...
    WebElement actualname = cpo.getVerifytext();
    String actualsubject = actualname.getText();
    System.out.println(actualsubject);
    if(actualsubject.equals(subject))
    {
    	Reporter.log("Purchase order "+actualsubject+" is successfully created.",true);
    }
    else
    {
    	Reporter.log("Purchase order creation is failed. ",true);
    }
}
}
