...
import com.Tig.pom.PurchaseOrderInformationPage;
import com.Tig.pom.VtigerHomePage;

public class CreateDuplicatePurchaseOrder1  extends Baseclass{
	@Test
	public void toCreateDuplicate1() throws EncryptedDocumentException, InvalidFormatException, IOException
	{
		...
		Reporter.log(" click on more dropdown",true);
		v.getPurchaseOrderlink().click();
		Reporter.log(" click on PurchaseOrder link",true);
		...
		Reporter.log(" click on Dulplicate button",true); 
		cpo.getSavebn().click();
		Reporter.log(" Click on save button",true);
		poi.getPurchaseOrderTab().click();
		Reporter.log(" Click on PurchaseOrder Tab",true);
