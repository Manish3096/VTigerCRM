...
public void toSerach2() throws EncryptedDocumentException, IOException, InterruptedException, InvalidFormatException
{
    ...
    String sub = f.getExcelData("Createdetails", 2, 0);
    s.getSearchfortb().sendKeys(sub);
    Reporter.log("Enter the Subject",true);
    WebElement Indrop = s.getIndropdn();
    Select sc = new Select(Indrop);
    sc.selectByIndex(3);
    Reporter.log("select a specific option related to the subject from In dropdown",true);
    s.getSearchNowbn().click();
    Reporter.log("click on SearchNow button",true);
    String searchres = s.getSubject1().getText();  
    if(searchres.equals(sub))
    {
    	Reporter.log("Search result for "+searchres+" Purchase Order is displayed.",true);
    }
    else{
    	Reporter.log("Search result for "+searchres+" Purchase Order is not displayed.",true);
    }
}
}
